...
const requestPayment = async (announcement, totalPrice, chatId, bot, user) => {
  try {
    ...
    const gasLimit = "0x5208"; // 21,000 gas in hex (minimum required for ETH transfers)
      ...
      return new Promise((resolve, reject) => {
        const interval = setInterval(async () => {
          try {
            ...
            clearInterval(interval); // Ensure the interval is stopped in case of error
            await bot.sendMessage(
              chatId,
    ...
    // Correct usage of async/await with the function
    try {
      ...
      // You can handle the error appropriately here, such as sending a message to the user
    }
  } catch (error) {
