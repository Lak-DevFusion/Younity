...
export const sendAnnouncmentInCommunityGroup = async (bot, hash) => {
  try {
    ...
    // Store message ID in the announcement document
...
export const setupReactionHandler = (bot) => {
  bot.on("callback_query", async (query) => {
    try {
      ...
      // Check if announcement has already been circulated
      ...
      // Check if user has already reacted (either liked or disliked)
      ...
      // Proceed with adding the reaction
      if (action === "l") {
        // Remove any existing dislikes from this user (just in case)
        ...
        // Only proceed if the announcement hasn't been circulated and meets the threshold
        if (
          ...
          const chatIds = ["-4530450065", "-4524819557"]; // Add the group chat IDs here
        ...
        // Remove any existing likes from this user (just in case)
        await Announcment.updateOne(
          { _id: announcement._id },
          {
